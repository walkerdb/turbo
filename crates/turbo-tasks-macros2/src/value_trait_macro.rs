...
pub fn value_trait(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl #ident for turbo_tasks::vc::Vc<dyn #ident>  {
            #(#trait_fns)*
        }
        ...
        impl<T> #ident for turbo_tasks::vc::Vc<T> where T: turbo_tasks::vc::AsDyn<dyn #ident> + ?Sized, turbo_tasks::TaskInput: for<'a> std::convert::From<&'a turbo_tasks::vc::Vc<T>> #(, #supertrait_refs: std::convert::From<turbo_tasks::vc::Vc<T>>)* {
            #(default #trait_fns)*
        }

        // impl turbo_tasks::vc::Value for dyn #ident {
        //     fn get_trait_type_id(&self) -> turbo_tasks::TraitTypeId {
        //         *#trait_type_id_ident
        //     }
        // }
